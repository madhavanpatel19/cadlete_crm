...
<script>
    $(document).ready(function() {
        ...
        function applyFilters() {
            ...
            $('.task-item').each(function() {
                ...
                // Get empId from the closest task list container
                ...
                if (filterDate) {
                    if (isCompleted) {
                        // For completed tasks, only show if they match the selected date
                        showTask = (taskDate === filterDate);
                    } else {
                        // Incomplete tasks ALWAYS show, regardless of date filter
                        showTask = true;
                    }
                }
            ...
            // Update completed headers for each employee based on visible count
            $('.todo-column').each(function() {
                const empId = $(this).data('emp-id');
                ...
                if (compCount > 0) {
                    ...
                    header.hide();
                    $(`#completed-tasks-${empId}`).hide(); // Auto collapse if 0
    ...
    function showInlineAddTask(empId) {
        $(`#inline-add-form-${empId}`).slideDown(200);
        $(`#inline-task-input-${empId}`).focus();

        const today = new Date();
        const yyyy = today.getFullYear();
        const mm = String(today.getMonth() + 1).padStart(2, '0');
        const dd = String(today.getDate()).padStart(2, '0');
        $(`#inline-task-date-${empId}`).val(`${yyyy}-${mm}-${dd}`);
    }

    function hideInlineAddTask(empId) {
        $(`#inline-add-form-${empId}`).slideUp(200);
        $(`#inline-task-input-${empId}`).val('');
        $(`#inline-task-date-${empId}`).val('');
        $(`#inline-task-priority-${empId}`).val('Medium');
    }

    function saveInlineTask(empId) {
        const name = $(`#inline-task-input-${empId}`).val().trim();
        const projId = 0; // No project selected for inline tasks
        const date = $(`#inline-task-date-${empId}`).val();
        const priority = $(`#inline-task-priority-${empId}`).val();
    ...
    function fetchProjectEmployees(projectId) {
        ...
        $.ajax({
            url: 'ajax/projects/ajax_get_project_employees.php',
            method: 'POST',
            data: {
    ...
    function loadTasks(empId) {
        // Pass project_id=0 to get ALL projects for this employee
        $.ajax({
    ...
    function renderTasks(empId, tasks) {
        ...
        // Re-apply filters after rendering new tasks
