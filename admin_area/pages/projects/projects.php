...
<script>
    ...
    $(document).ready(function() {
        ...
        const mediaDefinitions = {
            project_images: [
                { name: '3D CAD Images and Line Drawings', spec: 'From Drive' },
                { name: 'Final Product', spec: 'High Resolution' },
                { name: 'Photorealistic Renders from AI', spec: 'AI Renders' },
                { name: 'Work in Progress Images + Team photo with Final Product', spec: 'WIP + Team Photo' }
            ],
            social_media: [
                { name: 'Case Study Image/Portfolio Image', spec: '3840x2560' },
                { name: 'Cover Page Image for Instagram', spec: '4:5 Ratio' },
                { name: 'Carousel Images for Insta/Linkedin', spec: '4:5 Ratio' },
                { name: 'Square Portfolio Image', spec: '1:1 Ratio' },
                { name: 'Horizontal Portfolio Image', spec: '3403x1914' },
                { name: 'Cover Image for Portfolio', spec: '1280x769' },
                { name: 'Product Reel - 1 (Detailed)', spec: 'Instagram Reel' },
                { name: 'Product Reel - 1 (Very Short)', spec: 'Instagram Reel' }
            ]
        };
        ...
        window.switchRepoTab = function(tab) {
            ...
            // Toggle buttons
            $('#btn-add-artifact').toggle(tab === 'documents');
            $('#btn-add-link').toggle(tab === 'links');
            $('#btn-add-project-image').toggle(tab === 'project_images');
            $('#btn-add-social-post').toggle(tab === 'social_media');
        ...
        function refreshRepoContent() {
            ...
            if (currentRepoTab === 'documents') {
                url = 'ajax/projects/ajax_view_project_documents.php';
            } else if (currentRepoTab === 'links') {
                url = 'ajax/projects/ajax_view_project_links.php';
            } else if (currentRepoTab === 'project_images' || currentRepoTab === 'social_media') {
                url = 'ajax/projects/ajax_view_project_media.php';
        ...
        $('#add-media-form-unified').submit(function(e) {
            ...
            $.ajax({
                url: 'ajax/projects/ajax_add_project_media.php',
                method: 'POST',
                data: formData,
                processData: false,
                contentType: false,
        ...
        window.deleteMediaAsset = function(assetId) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: 'ajax/projects/ajax_delete_project_media.php',
                        method: 'POST',
