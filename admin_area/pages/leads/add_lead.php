    ...
    if (mysqli_query($con, $insert_lead)) {
        ...
        if (file_exists(__DIR__ . '/../../includes/notification_helper.php')) {
            ...
            foreach ($assigned_employees_arr as $eid) {
                $eid = intval($eid);
                if ($eid > 0) {
                    addSystemNotification('employee', $eid, "Lead Assignment: $client_name", "You have been assigned to lead '$client_name'.", "index.php?leads&open_lead=$lead_id_inserted", 'lead_assigned');
                }
            }
        }
        echo "<style>
