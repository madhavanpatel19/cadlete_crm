...
if (isset($_POST['ajax_delete_nda']) || isset($_GET['ajax_delete_nda'])) {
    $delete_id = isset($_POST['ajax_delete_nda'])
                            ...
                            <tr data-nda-row="<?php echo $id; ?>" style="transition: background 0.2s;">
                                ...
                                <td style="padding: 15px; text-align: center; border-bottom: 1px solid #f1f5f9;">
                                    <div style="display: flex; justify-content: center; gap: 8px;">
                                        <a href="pages/documents/generate_nda.php?id=<?php echo $id; ?>&action=view" target="_blank" class="btn-icon-premium btn-icon-view" title="Print/View">
...
<div class="modal fade" id="newNDAModal" tabindex="-1" role="dialog" aria-labelledby="newNDAModalLabel">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content" style="border-radius: 20px; border: none; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.25); overflow: hidden;">
            ...
            <form action="pages/documents/generate_nda.php" method="post" target="_blank">
                <div class="modal-body" style="padding: 30px; background: #fff;">
                    <div class="row">
                        <div class="col-md-6" style="margin-bottom: 20px;">
                            ...
                            <input type="text" name="name" class="form-control" placeholder="Full Name" required style="height: 50px; background: #f8fafc; border: 1.5px solid #e2e8f0; border-radius: 14px; padding: 12px 20px; width: 100%; color: #0f172a; font-weight: 600; outline: none; transition: all 0.3s;" onfocus="this.style.borderColor='#DF2127'; this.style.boxShadow='0 0 0 4px rgba(223, 33, 39, 0.1)';" onblur="this.style.borderColor='#e2e8f0'; this.style.boxShadow='none';">
                        </div>
                        <div class="col-md-6" style="margin-bottom: 20px;">
                ...
                <div class="modal-footer" style="padding: 20px 30px; background: #f8fafc; border-top: 1px solid #e2e8f0; border-radius: 0 0 20px 20px; text-align: right;">
                    ...
                    <button type="submit" class="btn-premium-add">
                        <i class="fa fa-plus"></i> Generate NDA
                    </button>
                </div>
            </form>
        </div>
...
<div class="premium-confirm-overlay" id="deleteConfirmOverlay">
    <div class="premium-confirm-modal">
        ...
        <h3 style="margin: 0 0 10px 0; font-weight: 700; color: #0f172a;">Delete NDA?</h3>
        <p style="margin: 0 0 25px 0; font-size: 14px; color: #64748b;">This action will permanently remove this NDA record. This cannot be undone.</p>
...
<script>
    ...
    confirmDeleteBtn.addEventListener('click', function() {
        ...
        fetch('view_nda.php', {
                ...
                body: `ajax_delete_nda=${encodeURIComponent(currentDeleteId)}`
            })
            .then(res => res.json())
            .then(result => {
                if (result.status === 'success') {
                    const row = document.querySelector(`[data-nda-row="${currentDeleteId}"]`);
                    if (row) {
                        row.remove();
                    }
                    closeDeleteConfirm();
                    showPremiumAlert('NDA deleted successfully');
                    return;
                }
            ...
            .catch(() => {
                showPremiumAlert('Error deleting NDA', 'error');
                confirmDeleteBtn.disabled = false;
                confirmDeleteBtn.innerHTML = 'Delete Now';
            });
    });
