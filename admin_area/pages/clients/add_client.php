...
if (isset($_POST['submit_client'])) {
    ...
    $client_image = handleClientImageUpload($_FILES['client_image']);

    $insert_client = "INSERT INTO clients (image, name, mobile, email, country, company_name, website, industry) 
                      VALUES ('$client_image', '$name', '$mobile', '$email', '$country', '$company_name', '$website', '$industry')";

    if (mysqli_query($con, $insert_client)) {
        $client_id = mysqli_insert_id($con);

        // Handle Project Info if provided
        ...
        if (!empty($project_name)) {
            ...
            $remark_text = isset($_POST['remark']) ? mysqli_real_escape_string($con, $_POST['remark']) : '';

            $insert_project = "INSERT INTO client_projects (client_id, project_name, project_date, budget) 
                               VALUES ('$client_id', '$project_name', '$project_date', '$budget')";

            if (mysqli_query($con, $insert_project)) {
                $project_id = mysqli_insert_id($con);
                if (!empty($remark_text)) {
                    $insert_remark = "INSERT INTO client_project_remarks (project_id, remark) VALUES ('$project_id', '$remark_text')";
...
if ($success): ?>
    ...
    <div class="success-modal-overlay" id="successModal">
        <div class="success-modal-content">
            ...
            <p class="success-message">Client <strong><?php echo htmlspecialchars($name); ?></strong> has been registered successfully.</p>
    ...
    <div class="add-client-wrapper">
        <form method="POST" id="add_client_form" enctype="multipart/form-data">
            ...
            <div class="section-header-custom">
                ...
                <div>
                    <h3 class="section-title-custom">Client Information</h3>
                    <p class="section-subtitle-custom">Basic details about the client</p>
                </div>
            </div>
                ...
                <div class="col-md-4">
                    <div class="form-group-custom" style="height: 100%;">
                        ...
                        <label class="upload-box" for="client_image">
                            ...
                            <div class="img-preview-wrapper" id="preview_wrapper">
                                <img id="client_preview" src="">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group-custom">
                                ...
                                <input type="text" name="name" class="p-input-premium" placeholder="Enter client name" required>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group-custom">
                                <label class="form-label-custom">Company Name <span class="req">*</span></label>
                                <input type="text" name="company_name" class="p-input-premium" placeholder="Type company name..." required>
                            </div>
                        </div>
                    </div>
                            ...
                            ?>
                                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 5px;">
                                        <label style="font-weight: 500; color: #475569; cursor: pointer; margin: 0; font-size: 13px;">
                                            <input type="checkbox" name="industry[]" value="<?php echo htmlspecialchars($i_name); ?>" style="margin-right: 8px; width: 14px; height: 14px; vertical-align: middle; accent-color: #DF2127;"> <?php echo htmlspecialchars($i_name); ?>
                                        </label>
