...
function add_user($con)
{
    ...
    if (!empty($adminTypedPassword)) {
        ...
        $plainPassword = $adminTypedPassword;
    } else {
    ...
    education_json, employment_json, account_name, bank_branch, account_number, account_type_ifsc, employee_image, offer_latter, NDA, Aadhar_card, Pan_card, Passportsize_photo, old_company_slary_slip)
              VALUES
    ('$name', '$contact', '$address', '$email', '$company_email', '$blood', '$gender', '$joinDate', '$department', '$designation', '$basic', '$hra', '$allowance', '$deductions', '$salary', '$passEscaped',
...
$next_id_q = mysqli_query($con, "SELECT AUTO_INCREMENT FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'emp_list'");
$next_emp_id = 1;
if ($next_id_q && $row_nid = mysqli_fetch_assoc($next_id_q)) {
    $next_emp_id = (int)$row_nid['AUTO_INCREMENT'];
} else {
    $max_id_q = mysqli_query($con, "SELECT MAX(id) as max_id FROM emp_list");
    if ($max_id_q && $row_mid = mysqli_fetch_assoc($max_id_q)) {
        $next_emp_id = ((int)$row_mid['max_id']) + 1;
    }
}
?>
...
<div class="page-wrapper premium-ui-enabled">
    <div class="page-header-premium">
        <div style="display: flex; align-items: center; gap: 12px;">
            <h1 style="margin: 0; font-size: 22px; font-weight: 800; color: #1e293b;"><i class="fa fa-user-plus" style="color: #dd2127;"></i> Add Employee</h1>
            <span style="background: #e2e8f0; color: #1e293b; font-size: 13px; font-weight: 800; padding: 4px 10px; border-radius: 6px; letter-spacing: 0.5px;">Series: <?php echo format_emp_id($next_emp_id); ?></span>
        </div>
        ...
        <div class="premium-card" style="margin: 0 30px 30px 30px; border-radius: 12px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05); background: #fff;">
            <div style="padding: 25px 30px; border-bottom: 1px solid #f1f5f9; display: flex; align-items: center; justify-content: space-between;">
                ...
                <div style="background: #f1f5f9; border: 1px solid #cbd5e1; padding: 6px 14px; border-radius: 8px; font-size: 13px; font-weight: 800; color: #334155;">
                    <i class="fa fa-id-card-o" style="color: #64748b; margin-right: 6px;"></i> Assigned ID: <span style="color: #dd2127;"><?php echo format_emp_id($next_emp_id); ?></span>
                </div>
            </div>
...
<script>
    ...
    function deleteDesignationOption(selectId = 'emp_designation') {
        ...
        for (let i = 0; i < select.options.length; i++) {
            ...
            if (val && val.trim() !== '') {
                options.push({ text: select.options[i].text, value: val });
            }
        }
